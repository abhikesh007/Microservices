...
public class OrderController {
	...
	public ResponseEntity<OrderResponse> placeOrder(@PathVariable("customerId") int customerId) {
		...
		if (customer != null) {
			...
			if (cart.getcartLineItems() != null && cart.getcartLineItems().size() > 0) {
				...
				cartServiceProxy.deleteCartItems(cart.getCartId());
	...
	public ResponseEntity<OrderResponse> getOrders(@PathVariable("customerId") int customerId) {
		...
		System.out.println(customerOrderRepo.findByCustomerId(customerId));
		if (customerOrderList != null && customerOrderList.size() > 0) {
			List<Integer> orderIdList = customerOrderList.stream().map(customerOrder -> customerOrder.getId().getOrderId())
					.collect(Collectors.toList());
			...
			System.out.println("res if: "+response);
		} else {
			response = new ResponseEntity<>(HttpStatus.NOT_FOUND);
			System.out.println("res else: "+response);
		}
		return response;
	}
}
