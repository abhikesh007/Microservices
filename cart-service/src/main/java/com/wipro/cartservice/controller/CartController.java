...
public class CartController {
    ...
    public ResponseEntity<Cart> deleteCartLineItemById(@PathVariable("id") int id) {
        ResponseEntity<Cart> response = null;
        Cart cart = service.getCartItemById(id);
        if (cart == null) {
            response = new ResponseEntity<>(HttpStatus.NOT_FOUND);
        } else {
            List<CartLineItem> lineItems = cart.getcartLineItems();
            cart.setcartLineItems(null);
            ;
            cart = service.saveOrUpdate(cart);
            lineItems.forEach(li -> {
                service.deleteCartLineItemById(li.getLid());
            });
            response = new ResponseEntity<>(cart, HttpStatus.ACCEPTED);
        }
        return response;
